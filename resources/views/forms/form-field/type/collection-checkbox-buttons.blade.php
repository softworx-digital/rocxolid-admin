<div class="row">
    <ul id="{{ $component->getDomId('append-custom-value-target') }}" class="list-group btn-group-vertical col-xs-12" data-toggle="buttons">
...
@if ($component->getOption('enable-custom-values'))
    ...
    @endforeach
        ...
        <div class="list-group-item no-top-border">
            <div class="row">
                <div class="col-xs-11">
                    {!! Form::text($component->getFormField()->getFieldName(), null, [ 'id' => $component->getDomId('append-custom-value'), 'class' => 'form-control input-sm no-border no-shadow', 'placeholder' => $component->translate('_custom-value') ]) !!}
                </div>
                <div class="col-xs-1">
                    <button
                        class="btn btn-primary btn-xs pull-right"
                        title="{{ $component->translate('_add') }}"
                        data-append-custom-value="{{ $component->getDomIdHash('append-custom-value') }}"
                        data-append-custom-value-target="{{ $component->getDomIdHash('append-custom-value-target') }}"
